# код функции convert_tsf_to_dataframe взят из репозитория Monash University Hugging Face https://huggingface.co/datasets/Monash-University/monash_tsf/blame/aeb8eee809d424a039784be7ad83312753b4524c/utils.py
...
def convert_tsf_to_long_dataframe(
    full_file_path_and_name,
    replace_missing_vals_with="NaN",
    value_column_name="series_value",
):
    loaded_data, frequency, forecast_horizon, contain_missing_values, contain_equal_length = convert_tsf_to_dataframe(full_file_path_and_name, replace_missing_vals_with, value_column_name)
    records = []
    ...
    for _, row in loaded_data.iterrows():
        ...
        for date, val in zip(dates, values):
            records.append({'unique_id': series_id, 'ds': date, 'y': val})
    return pd.DataFrame(records), frequency, forecast_horizon, contain_missing_values, contain_equal_length
